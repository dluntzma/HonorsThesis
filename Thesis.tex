...
\caption{The Ti:Sapphire laser emits $860$ nm light some of which is diverted to absolutely stabilize the using an iodine lock or an ultra low expansion optical cavity. The light is then passed through a doubler so the wavelength is $430$ nm and then another doubler so the wavelength is the required $215$ nm. The $215$ nm light is then coupled into an optical cavity within a vacuum chamber where the $215$ nm light is focused. The krypton atoms are directed through a capillary plate and through the optical cavity perpendicular to the $215$ nm light.}
...
where $\lambda$ is an eigenvalue of the matrix and $v$ is an eigenvector of the matrix. $A$ is the amplitude determined by the initial conditions. Our initial conditions are that all of the atoms begin in the ground state, state 1. That is
\begin{equation}
\label{InitialCond}
N_1(0) = 1, \	\	\	\	
N_2(0) = 0, \	\	\	\	
...
To avoid measuring the blue light with the power meter, we measured the beam diameter at various $z$ locations directly using a ruler with white card taped to it. This was a rougher measurement, much more susceptible to systematic error due to parallax when measuring the power and fitting the waists as described above. However it was a relatively simple measurement that we could preform quickly and have usable results. The values we obtained are in Fig.~\ref{WaistFit}. We found that the beam waist was $45 \pm 4$ cm before the mirror used to define the origin along the $z$ axis and that waist was $12.1 \pm 0.4 \mu$m with a divergence angle of $0.35 \pm 0.01^\circ$.
...
\begin{subfigure}{.7\textwidth}
  ...
  \label{WaistFitL}
\end{subfigure}
\caption{The data from measuring twice the effective beam waist (beam diameter) with a fit using Eq.~\ref{WaistDivergence}. The location of the beam waist is $-45 \pm 4$ cm, that is $45$ cm before the location defined as the origin. And the waist is $12.1 \pm 0.4 \mu$m with a divergence angle of $0.35 \pm 0.01^\circ$.}
\label{WaistFit}
\end{figure}
...
The next steps, and future theses, are to absolutely stabilize the laser as outlined in the absolute stabilization section, and to produce $215$ nm light from the laser. That $215$ nm light will then need to be characterized and those specifications sent to the company that will build the optical build up cavity. Once the cavity is obtained the the $215$ nm light will be coupled into it. Then a capillary plate designed to direct a flow of krypton atoms perpendicular to the light. Finally after producing the metastable krypton atoms, a magneto-optical trap will be set up to preform atom trap trace analysis.
